...
// 4. Generate Python module with embedded context for Pyodide runtime
const pyModule = `"""Auto-generated codebase context for the Deep Planner worker.

Generated by: npx tsx scripts/bundle-deep-planner-context.ts
Do not edit manually — re-run the bundle script to update.
"""

CLAUDE_MD = ${JSON.stringify(truncatedClaudeMd)}

SCHEMA_GRAPHQL = ${JSON.stringify(truncatedSchema)}

DB_SCHEMA = ${JSON.stringify(truncatedDbSchema)}
`;
const pyModulePath = join(ROOT, "workers", "deep-planner", "src", "context_data.py");
writeFileSync(pyModulePath, pyModule);
console.log(`Generated Python context module (${pyModule.length} chars)`);
...
console.log("Context files written to workers/deep-planner/context/ and src/context_data.py");
