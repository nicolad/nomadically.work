...
A Cloudflare Worker for inserting job listings into the Turso database.

## Features

- ✅ Bulk job insertion via POST requests
- ✅ Automatic upsert (insert or update if exists based on GUID)
- ✅ Input validation
...
npx wrangler secret put TURSO_DB_AUTH_TOKEN --name nomadically-work-insert-jobs

# Optional: Set API secret for authentication
npx wrangler secret put API_SECRET --name nomadically-work-insert-jobs
```
...
```
https://nomadically-work-insert-jobs.<your-subdomain>.workers.dev
```
...
Authorization: Bearer <your-api-secret>  # Optional, if API_SECRET is configured
```
  ...
  "jobs": [
    {
      ...
      "url": "https://jobs.acme.com/senior-frontend-dev",
    ...
    },
    {
      ...
      "location": "Remote",
      "url": "https://techstartup.com/careers/backend",
      "employmentType": "full-time",
...
{
  "success": true,
  "message": "Inserted 2/2 jobs",
  "data": {
    "totalJobs": 2,
    "successCount": 2,
    "failCount": 0,
    "successfulIds": [
      "Acme Corp_Senior Frontend Developer_1738713600000",
      "Tech Startup_Backend Engineer_1738713600001"
    ],
    "failures": []
  }
}
...
{
  "success": false,
  "message": "Inserted 1/2 jobs",
  "data": {
    "totalJobs": 2,
    "successCount": 1,
    "failCount": 1,
    "successfulIds": ["Acme Corp_Senior Frontend Developer_1738713600000"],
    "failures": ["Database constraint violation"]
  }
}
```
...
- `publishedDate`: When the job was published (ISO 8601 string)
- `sourceType`: Source type (string)
- `sourceCategory`: Source category (string)
- `sourceDetail`: Additional source details (string)
- `guid`: Unique identifier (string, auto-generated if not provided)
- `keywords`: Array of keywords (string[])
- `employmentType`: Employment type (string)
- `experienceLevel`: Required experience level (string)
- `techStack`: Array of technologies (string[])
- `status`: Job status (string, defaults to "new")
- `applied`: Whether you've applied (boolean)
- `appliedAt`: When you applied (ISO 8601 string)
- `isDeveloperRole`: Whether it's a developer role (boolean)
- `developerConfidence`: Confidence score (string)
- `remoteFriendly`: Whether remote work is supported (boolean)
...
```bash
curl -X POST https://nomadically-work-insert-jobs.<your-subdomain>.workers.dev \
...
const result = await insertJobs([
  {
    ...
    url: 'https://cloudservices.com/jobs/devops',
    location: 'Remote - EU',
    remoteFriendly: true,
...
console.log(`Successfully inserted ${result.data.successCount} jobs`);
```
...
result = insert_jobs([
    {
        ...
        'company': 'AI Startup',
        'url': 'https://aistartup.com/careers/ml',
        'location': 'Remote',
        'remoteFriendly': True,
...
print(f"Successfully inserted {result['data']['successCount']} jobs")
```
...
npx wrangler dev --config wrangler.insert-jobs.toml

# Test locally
curl -X POST http://localhost:8787 \
  -H "Content-Type: application/json" \
  -d '{"jobs": [{"title": "Test Job", "company": "Test Co", "url": "https://test.com"}]}'
```
...
```bash
# View live logs
npx wrangler tail --name nomadically-work-insert-jobs

# View deployment info
npx wrangler deployments list --name nomadically-work-insert-jobs
```

## Security

- Use the `API_SECRET` environment variable to require authentication
- The worker validates all input before inserting into the database
- CORS is enabled by default (customize if needed)
...
Set the API_SECRET if configured:

```bash
npx wrangler secret put API_SECRET --name nomadically-work-insert-jobs
```

### "TURSO_DB_AUTH_TOKEN is not defined"

Set the Turso auth token:
