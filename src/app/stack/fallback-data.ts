    ...
    entries: [
      {
        ...
        maturity: "mature",
        adoption_date: "2024-Q4",
        architecture_role: "Primary application framework — hosts the GraphQL API, serves the React frontend with SSR/RSC, and manages routing. All code runs through Next.js whether it's a page render or an API call.",
        depends_on: ["React 19", "Vercel"],
        depended_by: ["Apollo Server 5", "Clerk", "Radix UI", "Vercel AI SDK"],
        category_tags: ["framework", "frontend", "ssr", "rsc", "api"],
        ecosystem: [
          { name: "@as-integrations/next", role: "Apollo Server Next.js adapter", version: "3.2.0" },
          { name: "next-themes", role: "Theme management for dark mode", version: "0.4.4" },
          { name: "@clerk/nextjs", role: "Auth middleware and components" },
          { name: "@next/bundle-analyzer", role: "Bundle size analysis" },
        ],
        metrics: [
          { label: "API Routes", value: 41 },
          { label: "Pages", value: 11 },
          { label: "Providers", value: 5 },
          { label: "Max Duration", value: 60, unit: "s" },
        ],
        configuration_files: [
          { path: "next.config.ts", note: "Next.js configuration with redirects, image domains, and build settings" },
          { path: "vercel.json", note: "Vercel deployment config — route-level timeout at 60s" },
          { path: "tsconfig.json", note: "TypeScript config with @/* path alias mapping to ./src/*" },
        ],
        code_snippets: [
          {
            title: "RSC + Client Component boundary",
            description: "Server Components fetch data, Client Components handle interactivity",
            language: "typescript",
            path: "src/app/jobs/page.tsx",
            code: `// Server Component (no "use client") — runs on server\nexport default async function JobsPage() {\n  // Data fetching happens server-side\n  return <JobsSearchBar /> // Client Component for interactivity\n}`,
          },
          {
            title: "Provider composition pattern",
            description: "Each provider is a separate file composed in root layout",
            language: "typescript",
        ...
        testing_approach: [
          "Vitest for component and utility unit tests",
          "Eval tests in src/evals/ for LLM pipeline quality",
          "Preview deploys per PR enable manual E2E testing",
        ],
        learning_resources: [
          { title: "Next.js App Router Documentation", url: "https://nextjs.org/docs/app", type: "docs" },
          { title: "Server Components RFC", url: "https://github.com/reactjs/rfcs/pull/188", type: "article" },
          { title: "Vercel AI SDK with Next.js", url: "https://sdk.vercel.ai/docs/getting-started/nextjs", type: "docs" },
        ],
        version_history: [
          { version: "16.1.6", date: "2026-Q1", note: "Current version — stable App Router with full RSC support" },
          { version: "15.x", date: "2025-Q3", note: "Initial project version — migrated to 16 for improved RSC" },
        ],
        real_incidents: [
          {
            summary: "60s Vercel timeout killed bulk ATS enhancement mutations",
            resolution: "Moved long-running ATS API calls to Trigger.dev background tasks",
            severity: "medium",
          },
        ],
      },
      {
        ...
        maturity: "mature",
        adoption_date: "2024-Q4",
        architecture_role: "Rendering engine — all UI components are React components. RSC enables server-side rendering without client JS bundles for data-display pages.",
        depends_on: ["Next.js 16"],
        depended_by: ["Radix UI", "Clerk", "Apollo Client"],
        category_tags: ["ui", "rendering", "rsc", "hooks"],
        ecosystem: [
          { name: "react-dom", role: "DOM rendering target", version: "19.1.0" },
          { name: "react-hook-form", role: "Form state management" },
        ],
        metrics: [
          { label: "Client Components", value: "30+" },
          { label: "Server Components", value: "11" },
          { label: "Providers", value: 5 },
        ],
        code_snippets: [
          {
            title: "use client boundary decision",
            description: "Only interactive components get the 'use client' directive",
            language: "typescript",
            code: `"use client";\n// This component needs useState/useEffect for search state\nimport { useState } from "react";\n\nexport function JobsSearchBar() {\n  const [query, setQuery] = useState("");\n  return <input value={query} onChange={e => setQuery(e.target.value)} />;\n}`,
          },
        ],
        testing_approach: [
          "Vitest for component logic and hook tests",
          "Manual testing via Vercel preview deploys",
        ],
        learning_resources: [
          { title: "React 19 Documentation", url: "https://react.dev", type: "docs" },
          { title: "React Server Components", url: "https://react.dev/reference/rsc/server-components", type: "docs" },
        ],
      },
      {
        name: "Radix UI",
        ...
        maturity: "stable",
        adoption_date: "2024-Q4",
        architecture_role: "Design system layer — all UI primitives (cards, dialogs, badges, text, layout) come from Radix Themes. Icons from Radix Icons.",
        depends_on: ["React 19"],
        depended_by: [],
        category_tags: ["ui", "design-system", "accessibility", "components"],
        ecosystem: [
          { name: "@radix-ui/themes", role: "Complete design system", version: "3.2.1" },
          { name: "@radix-ui/react-icons", role: "Icon set", version: "1.3.2" },
          { name: "@radix-ui/colors", role: "Color system" },
        ],
        metrics: [
          { label: "Components Used", value: 15 },
          { label: "Icons Used", value: "20+" },
          { label: "Theme Colors", value: 8 },
        ],
        code_snippets: [
          {
            title: "Compound component pattern",
            description: "Radix primitives compose via dot notation",
            language: "tsx",
            code: `<Dialog.Root>\n  <Dialog.Trigger>\n    <Card style={{ cursor: "pointer" }}>\n      <Text>Click to open</Text>\n    </Card>\n  </Dialog.Trigger>\n  <Dialog.Content maxWidth="900px">\n    <Dialog.Title>Details</Dialog.Title>\n    <Dialog.Description>...</Dialog.Description>\n    <Dialog.Close><Badge>Close</Badge></Dialog.Close>\n  </Dialog.Content>\n</Dialog.Root>`,
          },
        ],
        learning_resources: [
          { title: "Radix Themes", url: "https://www.radix-ui.com/themes/docs/overview/getting-started", type: "docs" },
          { title: "Radix Primitives", url: "https://www.radix-ui.com/primitives/docs/overview/introduction", type: "docs" },
        ],
      },
      {
        name: "Clerk",
        ...
        maturity: "stable",
        adoption_date: "2024-Q4",
        architecture_role: "Authentication layer — handles sign-in/up, session management, and injects user identity into the GraphQL context for resolver-level authorization.",
        depends_on: ["Next.js 16", "React 19"],
        depended_by: ["Apollo Server 5"],
        category_tags: ["auth", "security", "identity", "middleware"],
        ecosystem: [
          { name: "@clerk/nextjs", role: "Next.js integration with middleware" },
        ],
        configuration_files: [
          { path: "src/middleware.ts", note: "Clerk middleware — route protection at the edge" },
          { path: "src/lib/admin.ts", note: "isAdminEmail() guard for mutation authorization" },
          { path: "src/lib/constants.ts", note: "ADMIN_EMAIL constant" },
        ],
        code_snippets: [
          {
            title: "Admin guard pattern",
            description: "Simple email-based authorization for mutations",
            language: "typescript",
            path: "src/lib/admin.ts",
            code: `import { isAdminEmail } from "@/lib/admin";\n\n// In resolver:\nif (!context.userId || !isAdminEmail(context.userEmail)) {\n  throw new Error("Forbidden");\n}`,
          },
        ],
        testing_approach: [
          "Manual testing via Clerk dashboard user management",
          "Admin guard tested implicitly via GraphQL mutation tests",
        ],
        gotchas: [
          "Admin role is checked by email comparison, not Clerk RBAC — works for single admin but doesn't scale",
          "Clerk middleware must be placed in src/middleware.ts (not src/app/) for proper route interception",
        ],
        learning_resources: [
          { title: "Clerk Next.js Quickstart", url: "https://clerk.com/docs/quickstarts/nextjs", type: "docs" },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        maturity: "mature",
        adoption_date: "2024-Q4",
        architecture_role: "API gateway — single GraphQL endpoint that unifies all data access. Resolvers compose DataLoaders, auth context, and Drizzle ORM to serve the frontend.",
        depends_on: ["Next.js 16", "Drizzle ORM", "Cloudflare D1", "Clerk"],
        depended_by: ["Apollo Client"],
        category_tags: ["api", "graphql", "backend", "schema-first"],
        ecosystem: [
          { name: "@apollo/server", role: "GraphQL server runtime", version: "5.3.0" },
          { name: "@as-integrations/next", role: "Next.js route handler adapter" },
          { name: "@graphql-tools/schema", role: "Schema merging and construction" },
          { name: "@graphql-codegen/cli", role: "Type generation from schema" },
          { name: "dataloader", role: "Batching and caching per-request", version: "2.2.3" },
          { name: "lodash.merge", role: "Deep-merging resolver maps" },
        ],
        metrics: [
          { label: "Schema Files", value: 13 },
          { label: "Resolver Modules", value: 15 },
          { label: "DataLoaders", value: 6 },
          { label: "Rate Limit", value: 100, unit: "req/min" },
        ],
        configuration_files: [
          { path: "codegen.ts", note: "GraphQL Code Generator config — outputs types, hooks, resolver types" },
          { path: "schema/", note: "Domain-split .graphql schema files" },
          { path: "src/apollo/context.ts", note: "GraphQL context factory — db, loaders, auth" },
          { path: "src/apollo/resolvers.ts", note: "Resolver composition via lodash.merge" },
        ],
        code_snippets: [
          {
            title: "GraphQL context injection",
            description: "Every resolver gets typed context with db, loaders, and auth",
            language: "typescript",
            path: "src/apollo/context.ts",
            code: `export interface GraphQLContext {\n  db: ReturnType<typeof drizzle>;\n  loaders: ReturnType<typeof createLoaders>;\n  userId: string | null;\n  userEmail: string | null;\n}`,
          },
          {
            title: "DataLoader batch pattern",
            description: "Batches DB queries to prevent N+1, limited to 100 for SQLite",
            language: "typescript",
            path: "src/apollo/loaders.ts",
            code: `const companyLoader = new DataLoader(\n  async (ids: readonly number[]) => {\n    // Batch size capped at 100 for D1's variable binding limit\n    const rows = await db.select().from(companies)\n      .where(inArray(companies.id, [...ids]));\n    return ids.map(id => rows.find(r => r.id === id) ?? null);\n  },\n  { maxBatchSize: 100 }\n);`,
          },
        ],
        testing_approach: [
          "Vitest eval tests validate GraphQL query responses",
          "GraphQL Playground at /api/graphql for manual testing",
          "Codegen verifies schema-type consistency at build time",
        ],
        real_incidents: [
          {
            summary: "N+1 queries on skills sub-field caused slow job listing responses",
            resolution: "Added DataLoader with batch size 100 for SQLite variable binding limit",
            severity: "medium",
          },
          {
            summary: "Edge Runtime incompatible with text-to-sql and execute-sql resolvers",
            resolution: "Switched GraphQL route to Node.js runtime, disabled incompatible resolvers on Edge",
            severity: "low",
          },
        ],
        learning_resources: [
          { title: "Apollo Server 5 Docs", url: "https://www.apollographql.com/docs/apollo-server/", type: "docs" },
          { title: "GraphQL Code Generator", url: "https://the-guild.dev/graphql/codegen", type: "docs" },
          { title: "DataLoader Pattern", url: "https://github.com/graphql/dataloader", type: "repo" },
        ],
        version_history: [
          { version: "5.3.0", date: "2025-Q4", note: "Current — stable with Next.js integration" },
          { version: "4.x", note: "Previous major — migrated to v5 for better TypeScript support" },
        ],
      },
      {
        name: "Vercel",
        ...
        maturity: "stable",
        adoption_date: "2024-Q4",
        architecture_role: "Hosting and deployment platform — serves the Next.js app, manages serverless functions, and provides the edge network for static assets and RSC payloads.",
        depends_on: ["Next.js 16"],
        depended_by: [],
        category_tags: ["hosting", "deployment", "serverless", "edge"],
        configuration_files: [
          { path: "vercel.json", note: "Route-level timeout configuration (60s max)" },
          { path: "scripts/deploy.ts", note: "Custom deploy script wrapping Vercel CLI" },
        ],
        metrics: [
          { label: "Max Duration", value: 60, unit: "s" },
          { label: "Regions", value: "auto" },
        ],
        real_incidents: [
          {
            summary: "60s timeout killed long-running ATS enhancement mutations",
            resolution: "Offloaded to Trigger.dev background tasks",
            severity: "medium",
          },
        ],
        learning_resources: [
          { title: "Vercel Documentation", url: "https://vercel.com/docs", type: "docs" },
          { title: "Vercel Functions Limits", url: "https://vercel.com/docs/functions/configuring", type: "docs" },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "Primary datastore — all persistent data (jobs, companies, skills, applications, contacts, ATS sources) lives in D1. Accessed by Workers directly via bindings and by Next.js via the Gateway Worker.",
        depends_on: [],
        depended_by: ["Drizzle ORM", "D1 Gateway Worker", "janitor", "process-jobs"],
        category_tags: ["database", "sqlite", "edge", "cloudflare"],
        ecosystem: [
          { name: "drizzle-orm/d1", role: "D1 driver for Drizzle ORM" },
          { name: "wrangler", role: "D1 management CLI" },
        ],
        metrics: [
          { label: "Tables", value: "10+" },
          { label: "Migration Files", value: "15+" },
          { label: "Free Reads/Day", value: "100K" },
          { label: "Storage Limit", value: 5, unit: "GB" },
        ],
        configuration_files: [
          { path: "src/db/schema.ts", note: "Drizzle schema — all table definitions" },
          { path: "migrations/", note: "SQL migration files applied via Wrangler" },
          { path: "drizzle.config.ts", note: "Drizzle Kit migration configuration" },
          { path: "wrangler.d1-gateway.toml", note: "D1 binding configuration for gateway" },
        ],
        code_snippets: [
          {
            title: "D1 HTTP client initialization",
            description: "Factory function with singleton caching for non-Worker access",
            language: "typescript",
            path: "src/db/d1-http.ts",
            code: `import { drizzle } from "drizzle-orm/d1";\nimport { createD1HttpClient } from "@/db/d1-http";\n\n// Cast as any — D1HttpClient implements subset of CF binding\nconst db = drizzle(createD1HttpClient() as any);`,
          },
        ],
        testing_approach: [
          "Drizzle Studio for interactive data inspection",
          "Migration files tested by applying locally before pushing to remote",
          "GraphQL resolver tests implicitly validate D1 queries",
        ],
        real_incidents: [
          {
            summary: "Migration from Turso to D1 required full data access layer rewrite",
            resolution: "Replaced @libsql/client with D1 HTTP client, built gateway worker",
            severity: "high",
          },
          {
            summary: "Boolean columns returning 0/1 instead of true/false broke GraphQL responses",
            resolution: "Added manual coercion in resolvers and { mode: 'boolean' } in schema",
            severity: "low",
          },
        ],
        version_history: [
          { version: "D1", date: "2025-Q1", note: "Current — migrated from Turso for native Worker bindings" },
          { version: "Turso/libSQL", date: "2024-Q4", note: "Original database — replaced due to Worker integration friction" },
        ],
        learning_resources: [
          { title: "Cloudflare D1 Documentation", url: "https://developers.cloudflare.com/d1/", type: "docs" },
          { title: "D1 with Drizzle ORM", url: "https://orm.drizzle.team/docs/get-started/d1-new", type: "docs" },
        ],
      },
      {
        name: "Drizzle ORM",
        ...
        maturity: "mature",
        adoption_date: "2024-Q4",
        architecture_role: "Query builder and schema management — all DB access goes through Drizzle. Schema is the single source of truth for TypeScript types and SQL migrations.",
        depends_on: ["Cloudflare D1"],
        depended_by: ["Apollo Server 5", "Trigger.dev"],
        category_tags: ["orm", "type-safe", "query-builder", "migrations"],
        ecosystem: [
          { name: "drizzle-orm", role: "Core ORM library", version: "0.38.4" },
          { name: "drizzle-kit", role: "Migration generation CLI" },
          { name: "drizzle-orm/d1", role: "D1 SQLite driver" },
          { name: "drizzle-orm/sqlite-core", role: "SQLite column types and helpers" },
        ],
        metrics: [
          { label: "Schema Tables", value: "10+" },
          { label: "Bundle Size", value: "~50KB" },
          { label: "Type Inference", value: "100%" },
        ],
        configuration_files: [
          { path: "src/db/schema.ts", note: "Complete database schema with all table definitions" },
          { path: "drizzle.config.ts", note: "Drizzle Kit config for migration generation" },
        ],
        code_snippets: [
          {
            title: "hasMore pagination pattern",
            description: "Fetch limit+1 to detect next page without COUNT query",
            language: "typescript",
            code: `const rows = await db.select().from(jobs)\n  .where(eq(jobs.is_remote_eu, true))\n  .orderBy(desc(jobs.posted_at))\n  .limit(limit + 1)\n  .offset(offset);\nconst hasMore = rows.length > limit;\nreturn { items: rows.slice(0, limit), hasMore };`,
          },
          {
            title: "Subquery composition for skill filtering",
            description: "Type-safe IN subquery using Drizzle builder",
            language: "typescript",
            code: `const skillFilter = inArray(\n  jobs.id,\n  db.select({ job_id: jobSkillTags.job_id })\n    .from(jobSkillTags)\n    .where(inArray(jobSkillTags.tag, skills))\n    .groupBy(jobSkillTags.job_id)\n    .having(sql\`count(distinct \${jobSkillTags.tag}) = \${skills.length}\`)\n);`,
          },
        ],
        testing_approach: [
          "Type inference validates schema-code alignment at compile time",
          "Migration files reviewed before applying",
          "Drizzle Studio for interactive query testing",
        ],
        learning_resources: [
          { title: "Drizzle ORM Overview", url: "https://orm.drizzle.team/docs/overview", type: "docs" },
          { title: "Drizzle SQLite Guide", url: "https://orm.drizzle.team/docs/get-started/d1-new", type: "docs" },
          { title: "Drizzle Queries", url: "https://orm.drizzle.team/docs/select", type: "docs" },
        ],
      },
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "Database access bridge — HTTP proxy that enables Vercel (external) to reach D1 (internal CF binding). Critical infrastructure path for all production queries.",
        depends_on: ["Cloudflare D1"],
        depended_by: ["Apollo Server 5", "Next.js 16"],
        category_tags: ["worker", "proxy", "gateway", "cloudflare"],
        configuration_files: [
          { path: "wrangler.d1-gateway.toml", note: "Worker config with D1 binding and API_KEY secret" },
          { path: "workers/d1-gateway.ts", note: "Gateway implementation — batch support, auth" },
          { path: "src/db/d1-http.ts", note: "Client-side HTTP client for calling the gateway" },
        ],
        security_considerations: [
          "CORS is currently set to * — needs tightening to Vercel domain only",
          "API key auth via CF Worker secrets — rotatable without code deploy",
          "No query validation — any SQL can be executed through the gateway",
        ],
        performance_notes: [
          "Adds ~20ms latency per hop (Vercel → CF Worker → D1)",
          "Batch queries reduce round trips — send N queries in 1 HTTP request",
          "Worker cold starts are <5ms on Cloudflare's edge network",
        ],
        gotchas: [
          "CORS * means any origin can call the gateway if they have the API key",
          "D1 binding name must match exactly between wrangler.toml and worker code",
          "Batch responses return in order — client must match responses to queries by index",
        ],
        learning_resources: [
          { title: "Cloudflare Workers", url: "https://developers.cloudflare.com/workers/", type: "docs" },
          { title: "D1 Bindings", url: "https://developers.cloudflare.com/d1/worker-api/", type: "docs" },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "Classification engine — determines whether jobs are genuinely remote-EU-eligible. The core ML decision that drives the entire job board's value proposition.",
        depends_on: ["Langfuse"],
        depended_by: [],
        category_tags: ["ai", "classification", "llm", "cost-optimized"],
        metrics: [
          { label: "Cost vs GPT-4", value: "~10x", unit: "cheaper" },
          { label: "Accuracy Bar", value: 80, unit: "%" },
          { label: "Latency", value: "2-3", unit: "s/job" },
          { label: "Cron Interval", value: 6, unit: "hours" },
        ],
        code_snippets: [
          {
            title: "Classification prompt pattern",
            description: "Schema-constrained output with strict JSON validation",
            language: "python",
            code: `# Prompt returns structured JSON:\n{\n  "is_remote_eu": true,\n  "confidence": 0.92,\n  "reasoning": "Job explicitly mentions EU timezones...",\n  "signals": ["eu-timezone", "no-visa-restriction"]\n}`,
          },
        ],
        testing_approach: [
          "Langfuse eval dataset with labeled jobs (obvious + edge cases)",
          "Accuracy must be >= 80% before any prompt/model change ships",
          "Vitest eval tests wrap classification logic for CI gating",
        ],
        learning_resources: [
          { title: "DeepSeek API Docs", url: "https://platform.deepseek.com/api-docs", type: "docs" },
      ...
      },
      {
        ...
        maturity: "adopted",
        adoption_date: "2025-Q1",
        architecture_role: "Agent runtime — powers SQL generation, strategy enforcement, architecture analysis, and code review agents. Uses MCP tools for typed tool definitions.",
        depends_on: ["Vercel AI SDK"],
        depended_by: [],
        category_tags: ["ai", "agents", "mcp", "reasoning"],
        ecosystem: [
          { name: "@ai-sdk/anthropic", role: "Anthropic provider for AI SDK" },
          { name: "@anthropic-ai/sdk", role: "Direct Claude API client" },
        ],
        configuration_files: [
          { path: "src/anthropic/", note: "Claude client, MCP configuration, sub-agents" },
          { path: "src/agents/", note: "Agent definitions (SQL, admin, strategy enforcer)" },
          { path: "src/tools/database/", note: "MCP-compatible database tools" },
        ],
        code_snippets: [
          {
            title: "MCP tool definition",
            description: "Typed tool with JSON Schema for database introspection",
            language: "typescript",
            path: "src/tools/database/",
            code: `// Tools let agents query schema metadata\nconst introspectTool = {\n  name: "introspect_database",\n  description: "Get table names and column info",\n  parameters: { type: "object", properties: {\n    table: { type: "string" }\n  }}\n};`,
          },
        ],
        learning_resources: [
          { title: "Anthropic API Docs", url: "https://docs.anthropic.com/", type: "docs" },
          { title: "Claude Agent SDK", url: "https://docs.anthropic.com/en/docs/agents-and-tools", type: "docs" },
      ...
      },
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "AI abstraction layer — unified interface for calling any LLM provider. Enables multi-model routing without provider-specific code.",
        depends_on: [],
        depended_by: ["Anthropic Claude", "DeepSeek", "OpenRouter"],
        category_tags: ["ai", "sdk", "multi-model", "streaming"],
        ecosystem: [
          { name: "ai", role: "Core AI SDK package" },
          { name: "@ai-sdk/anthropic", role: "Anthropic provider" },
          { name: "@ai-sdk/openai", role: "OpenAI/DeepSeek provider" },
          { name: "@ai-sdk/google", role: "Google/Gemini provider" },
        ],
        learning_resources: [
          { title: "Vercel AI SDK Docs", url: "https://sdk.vercel.ai/docs", type: "docs" },
          { title: "Tool Use Guide", url: "https://sdk.vercel.ai/docs/ai-sdk-core/tools-and-tool-calling", type: "docs" },
          { title: "Structured Output", url: "https://sdk.vercel.ai/docs/ai-sdk-core/generating-structured-data", type: "docs" },
        ],
      },
      {
        name: "OpenRouter",
        ...
        maturity: "adopted",
        category_tags: ["ai", "gateway", "multi-model", "fallback"],
      },
      {
        name: "Google ADK",
        ...
        adoption_date: "2025-Q4",
        category_tags: ["ai", "agents", "experimental", "multi-agent"],
      },
    ],
  },
  {
    ...
    entries: [
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "LLM observability hub — all LLM calls are traced, prompts are version-controlled, and eval scores are tracked over time. Central to the eval-first development strategy.",
        depends_on: [],
        depended_by: ["DeepSeek", "Anthropic Claude"],
        category_tags: ["observability", "llm", "tracing", "eval", "prompts"],
        ecosystem: [
          { name: "langfuse", role: "Core SDK for tracing and scoring" },
          { name: "langfuse-langchain", role: "LangChain integration callback" },
        ],
        configuration_files: [
          { path: "scripts/eval-remote-eu-langfuse.ts", note: "Eval script using Langfuse datasets and scoring" },
        ],
        metrics: [
          { label: "Accuracy Bar", value: 80, unit: "%" },
          { label: "Prompt Versions", value: "5+" },
          { label: "Eval Dataset Size", value: "50+" },
        ],
        code_snippets: [
          {
            title: "Eval-first workflow",
            description: "Every prompt change tested against labeled dataset",
            language: "typescript",
            code: `// 1. Fetch labeled dataset from Langfuse\nconst dataset = await langfuse.getDataset("remote-eu-classification");\n\n// 2. Run classification on each item\nfor (const item of dataset.items) {\n  const result = await classify(item.input);\n  // 3. Score accuracy\n  trace.score({ name: "accuracy", value: result === item.expected ? 1 : 0 });\n}`,
          },
        ],
        learning_resources: [
          { title: "Langfuse Documentation", url: "https://langfuse.com/docs", type: "docs" },
          { title: "Langfuse Tracing", url: "https://langfuse.com/docs/tracing", type: "docs" },
          { title: "Langfuse Datasets", url: "https://langfuse.com/docs/datasets/overview", type: "docs" },
        ],
      },
      {
        name: "LangSmith",
        ...
        category_tags: ["observability", "langchain", "tracing", "python"],
        depends_on: [],
        depended_by: ["process-jobs"],
        learning_resources: [
          { title: "LangSmith Documentation", url: "https://smith.langchain.com/", type: "docs" },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        maturity: "stable",
        architecture_role: "Scheduler — triggers the daily ingestion pipeline by scanning ATS sources and enqueuing work. No data processing, only orchestration.",
        depends_on: ["Cloudflare D1"],
        depended_by: ["insert-jobs"],
        category_tags: ["worker", "cron", "scheduler", "cloudflare"],
        configuration_files: [
          { path: "wrangler.toml", note: "Cron trigger at midnight UTC" },
        ],
      },
      {
        name: "insert-jobs",
        ...
        maturity: "stable",
        architecture_role: "Data ingestion — fetches job listings from ATS APIs and persists to D1. Queue-based for automatic retry and backpressure.",
        depends_on: ["janitor", "Cloudflare D1"],
        depended_by: ["process-jobs"],
        category_tags: ["worker", "queue", "ingestion", "cloudflare"],
        configuration_files: [
          { path: "wrangler.insert-jobs.toml", note: "Queue consumer configuration" },
        ],
        gotchas: [
          "Still has Turso references in some code paths (legacy from migration)",
          "No rate limiting per ATS API — could trigger API throttling on high volume",
        ],
      },
      {
        name: "process-jobs",
        ...
        maturity: "stable",
        architecture_role: "Classification engine runtime — LangGraph state machine that determines remote-EU eligibility. The critical ML decision point in the pipeline.",
        depends_on: ["DeepSeek", "Cloudflare D1", "LangSmith"],
        depended_by: [],
        category_tags: ["worker", "python", "classification", "langgraph", "cloudflare"],
        configuration_files: [
          { path: "workers/process-jobs/wrangler.jsonc", note: "Python Worker config with cron + queue triggers" },
        ],
        metrics: [
          { label: "Cron Interval", value: 6, unit: "hours" },
          { label: "Pipeline Nodes", value: 4 },
        ],
      },
      {
        name: "ashby-crawler",
      ...
      },
      {
        ...
        maturity: "adopted",
        adoption_date: "2025-Q3",
        architecture_role: "Resume matching — uses vector similarity to rank jobs against uploaded resumes. Enables personalized job recommendations.",
        depends_on: ["Cloudflare D1"],
        depended_by: [],
        category_tags: ["worker", "python", "rag", "vectors", "cloudflare", "ai"],
        ecosystem: [
          { name: "Cloudflare Vectorize", role: "Managed vector database" },
          { name: "Workers AI", role: "Free embedding generation" },
        ],
        configuration_files: [
          { path: "workers/resume-rag/wrangler.jsonc", note: "Python Worker config with Vectorize binding" },
        ],
        metrics: [
          { label: "Embedding Cost", value: "Free" },
          { label: "Vector DB", value: "Vectorize" },
        ],
        learning_resources: [
    ...
    entries: [
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "Background task runtime — executes long-running operations that exceed Vercel's 60s timeout. Primary use: ATS job enhancement and skill extraction.",
        depends_on: ["Drizzle ORM", "Cloudflare D1"],
        depended_by: [],
        category_tags: ["background-jobs", "queues", "retry", "dashboard"],
        ecosystem: [
          { name: "@trigger.dev/sdk", role: "Task SDK (v3)", version: "3.x" },
          { name: "@trigger.dev/react-hooks", role: "React hooks for task status" },
        ],
        configuration_files: [
          { path: "trigger.config.ts", note: "Task registration and runtime config" },
          { path: "src/trigger/", note: "Task definitions (enhancement, skill extraction)" },
        ],
        code_snippets: [
          {
            title: "Trigger.dev task pattern",
            description: "Task with lazy DB init, retry config, and error handling",
            language: "typescript",
            path: "src/trigger/",
            code: `export const enhanceJob = task({\n  id: "enhance-job",\n  maxDuration: 120,\n  retry: { maxAttempts: 3, factor: 2 },\n  queue: { concurrencyLimit: 5 },\n  run: async (payload) => {\n    const db = getDb(); // Lazy init\n    logger.info("Enhancing", payload);\n    // ... fetch from ATS API, update D1\n  },\n  handleError: async (payload, error) => {\n    if (error.message.includes("404"))\n      return { skipRetrying: true }; // Terminal error\n  },\n});`,
          },
        ],
        testing_approach: [
          "Trigger.dev dashboard for monitoring task execution and failures",
          "handleError pattern prevents wasting retry budget on terminal errors",
        ],
        learning_resources: [
          { title: "Trigger.dev Tasks Overview", url: "https://trigger.dev/docs/tasks-overview", type: "docs" },
          { title: "Trigger.dev v3 SDK", url: "https://trigger.dev/docs/v3", type: "docs" },
        ],
      },
    ],
  },
    ...
    entries: [
      {
        ...
        maturity: "stable",
        adoption_date: "2025-Q1",
        architecture_role: "Quality gate — enforces the eval-first policy by requiring accuracy benchmarks before any prompt or model change ships to production.",
        depends_on: ["Langfuse"],
        depended_by: [],
        category_tags: ["evaluation", "testing", "accuracy", "llm"],
        configuration_files: [
          { path: "scripts/eval-remote-eu-langfuse.ts", note: "Main eval script — runs classification against labeled dataset" },
        ],
        metrics: [
          { label: "Accuracy Bar", value: 80, unit: "%" },
          { label: "Dataset Items", value: "50+" },
        ],
        learning_resources: [
          { title: "Langfuse Evaluation", url: "https://langfuse.com/docs/scores/overview", type: "docs" },
        ],
      },
      {
        name: "Vitest",
        ...
        maturity: "stable",
        adoption_date: "2024-Q4",
        architecture_role: "Test runner — executes unit tests, eval tests, and CI quality gates. The eval-as-test pattern means LLM accuracy checks run alongside traditional tests.",
        depends_on: [],
        depended_by: [],
        category_tags: ["testing", "evaluation", "ci", "esm"],
        configuration_files: [
          { path: "vitest.config.ts", note: "Vitest configuration with path aliases" },
        ],
        code_snippets: [
          {
            title: "Eval-as-test pattern",
            description: "LLM evaluation logic wrapped in Vitest test cases",
            language: "typescript",
            path: "src/evals/",
            code: `import { describe, it, expect } from "vitest";\n\ndescribe("remote-eu classification", () => {\n  it("classifies obvious remote EU jobs correctly", async () => {\n    const result = await classify(sampleJob);\n    expect(result.is_remote_eu).toBe(true);\n    expect(result.confidence).toBeGreaterThan(0.8);\n  });\n});`,
          },
        ],
        learning_resources: [
          { title: "Vitest Documentation", url: "https://vitest.dev/", type: "docs" },
          { title: "Vitest with TypeScript", url: "https://vitest.dev/guide/", type: "docs" },
        ],
      },
    ],
  },
